...
#define COLOR_DOD_GREEN	Color(76, 102, 76, 255)
...
{
	char *pszCommandName;	// console command that will produce the voice command

	char *pszSoundName;		// name of sound to play

	PlayerAnimEvent_t iHandSignal;		//index into the hand signal array

	char *pszAlliedSubtitle;	// subtitles for each nationality
	char *pszAxisSubtitle;
	char *pszBritishSubtitle;
...
typedef struct DodHandSignal_s
{
	...
	char *pszSubtitle;	// subtitles for each nationality
...
enum
{
	...
	NUM_DOD_ACHIEVEMENTS	// must fit in a BYTE
};

#define ACHIEVEMENT_NUM_CONSECUTIVE_HEADSHOTS 5
#define ACHIEVEMENT_NUM_DEPLOYED_KILLS 10
